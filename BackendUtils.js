...
static async getLuckySpin(req, res) {
  try {
    const spinItem = SharedData.PurchasableItems.find(i =>
      i?.Name === 'LuckySpin' ||
      i?.Type === 'LuckySpin'
    );

    if (!spinItem) {
      return res.status(404).json({ error: 'SPIN_NOT_CONFIGURED' });
    }

    const rewards =
      spinItem.SpinRewards ||
      spinItem.spinRewards ||
      spinItem.rewards ||
      [];

    if (!Array.isArray(rewards) || rewards.length === 0) {
      return res.status(500).json({ error: 'SPIN_REWARDS_EMPTY' });
    }
  ...
  try {
    ...
    const totalWeight = rewardsList.reduce((sum, r) => {
      return sum + (r.Weight || r.weight || 1);
    }, 0);
    ...
    if (
      ...
      if (cosmeticId) {
        await UserModel.addSkin(user.stumbleId, cosmeticId);
      }
    }
...
class FriendsController {
  ...
  static async search(req, res) {
    try {
      ...
      if (!UserName) {
        return res.status(400).json({ ResultCode: -1, Message: 'UserName is required' });
      }
      ...
      // Search for exact match in username, premiumUsername, or adminUsername (case insensitive)
      ...
      if (!user) {
        Console.log('Friends', `User "${UserName}" not found`);
        return res.status(404).json({ ResultCode: -1, Message: 'User not found' });
      }

      Console.log('Friends', `Found user: ${user.username || user.premiumUsername || user.adminUsername}`);

      // Ensure userProfile exists and has correct username
      let userProfile = user.userProfile;
      if (!userProfile) {
        // Fallback: construct basic userProfile if missing
        userProfile = {
        ...
        // Update userName to the correct display name
...
class TournamentXController {
  static tournaments = [
    {
      ...
      nameKey: " BlockDash 1v1",
      descriptionKey: ".GG/SGPRISM",
      ...
      prizeBannerColour: "#747474",
      headerColour: "#747474",
      mapListGradientColourTop: "#747474",
      mapListGradientColourBottom: "#747474",
      listPriority: 0,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      prohibitedEmotes: [8, 13, 156, 123, 124, 174],
      detailsPanelBorderColourTop: "#747474",
      detailsPanelBorderColourBottom: "#D34286",
      colourData: {
        detailsPanelMainColour: "#747474",
        detailsPanelBorderColour: "#747474",
        headerGradientRight: "#747474",
        headerGradientLeft: "#747474",
        infoWidgetsGradientRight: "#747474",
        infoWidgetsGradientLeft: "#747474",
        infoWidgetsBorderColour: "#747474"
      },
      rounds: [
        {
          roundOrder: 1,
    ...
    id: 2,
      ...
      prohibitedEmotes: [8, 13, 156, 123, 124, 174],
      detailsPanelBorderColourTop: "#747474",
      detailsPanelBorderColourBottom: "#747474",
      colourData: {
        detailsPanelMainColour: "#747474",
        detailsPanelBorderColour: "#747474",
        headerGradientRight: "#747474",
    ...
    },
    {
      ...
      entryCurrencyCost2: 0,
      areEmotesRestricted: false,
      prohibitedEmotes: [8, 9, 13, 85, 156, 123, 124, 174],
      detailsPanelBorderColourTop: "#747474",
      detailsPanelBorderColourBottom: "#747474",
      colourData: {
        detailsPanelMainColour: "#747474",
      ...
      rounds: [
        {
          ...
          maxPlayersPerMatch: 2,
          areLevelsRestricted: true,
          permittedLevels: ["level21_pillar"]
        }
      ],
      awards: [
        {
      ...
      rounds: [
        {
          ...
          areLevelsRestricted: true,
          areEmotesRestricted: true,
          prohibitedEmotes: true [8, 9, 13, 85, 156, 123, 124, 174],
  ...
  static getActive(req, res) {
    try {
      ...
      // Retorna apenas o array de torneios (sem wrapper)
...
static async finish(req, res) {
  try {
    ...
    // Forward to RoundController.finishRoundV4 so the client receives the correct
    // ClientViewPayload; pass tournament hasBots flag to disable bots for this tournament
    ...
    // prevent finishRoundV4 from re-applying rewards (we already applied them above)
...
function filterByVersion(obj, maxVersion) {
  if (Array.isArray(obj)) {
    return obj.filter(item => {
      if (item.Version && versionCompare(item.Version, maxVersion) > 0) {
        return false;
      }
      return true;
    }).map(item => filterByVersion(item, maxVersion));
  } else if (obj && typeof obj === 'object') {
    const filtered = {};
    for (const key in obj) {
      if (obj[key] && typeof obj[key] === 'object') {
        filtered[key] = filterByVersion(obj[key], maxVersion);
      } else {
